...
const steps: readonly Step[] = [
  {
    ...
    name: "Step 1",
    title: "Create It",
    description: "AI-Powered Document Generation leveraging cutting-edge LLMs for instant legal drafting.",
  },
  {
    id: "2",
    name: "Step 2",
    title: "Own It",
    description: "IP Registration & Licensing on Story Protocol for immutable ownership records.",
  },
  {
    id: "3",
    name: "Step 3",
    title: "Prove It",
    description: "Evidence Validation via Constellation Network with multi-witness verification.",
  },
  {
    id: "4",
    name: "Step 4",
    title: "Monetize It",
    description: "Bitcoin Payments for seamless, permissionless transactions globally.",
  },
]
...
function FeatureCard({ children, step }: { children: React.ReactNode; step: number }) {
  ...
  return (
    ...
    >
      <motion.div
        className="pointer-events-none absolute -inset-px opacity-0 transition-all duration-300 group-hover:opacity-100"
        style={{
          background: useMotionTemplate`radial-gradient(650px circle at ${mouseX}px ${mouseY}px, hsl(var(--bitcoin-orange) / 20%), transparent 80%)`,
        }}
      />
      <div className="relative w-full overflow-hidden border border-neutral-200 bg-background transition-colors duration-300 dark:border-neutral-800">
         <div
          className="absolute inset-0"
          style={{
            backgroundImage: `radial-gradient(circle, hsl(var(--border) / 50%) 1px, transparent 1px)`,
            backgroundSize: '2rem 2rem',
          }}
        />
        <div className="relative m-10 min-h-[450px] w-full">
          <AnimatePresence mode="wait">
            ...
            >
              <motion.div
                className="text-sm font-semibold uppercase tracking-wider text-bitcoin"
                initial={{ opacity: 0, x: -20 }}
                animate={{ opacity: 1, x: 0 }}
                    ...
                    return (
                        <motion.li key={step.name} initial="inactive" animate={isCurrent ? "active" : "inactive"} variants={stepVariants} transition={{ duration: 0.3 }} className="relative" >
                            <button
                                type="button"
                                className={cn(
                                    "group flex items-center gap-2.5 px-3.5 py-1.5 text-sm font-medium transition-colors duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-bitcoin dark:focus-visible:ring-offset-black",
                                    isCurrent 
                            ...
                            >
                                <span className={cn(
                                    ...
                                    isCompleted 
                                        ? "bg-bitcoin text-primary-foreground" 
                                        : isCurrent 
                                            ? "bg-bitcoin/80 text-primary-foreground" 
...
const defaultClasses = {
  img: "border border-neutral-200 dark:border-neutral-800 shadow-2xl shadow-black/10 dark:shadow-neutral-950/50",
...
export function FeatureCarousel({
  ...
  return (
    ...
    >
        ...
        <motion.div variants={inViewVariants} transition={{ delay: 0.2 }}>
